						...
						<div class="card-body">
							<form action="{{ route('business_settings.update') }}" method="POST"
								...
								<div class="form-group mb-0">
									<label>{{ translate('Contact Info Groups') }} ({{ translate('Translatable') }})</label>
									<div class="contact-address-target">
										...
										@if ($max_count > 0)
											@for ($i = 0; $i < $max_count; $i++)
												@php
													$addr_val = isset($addresses[$i]) ? $addresses[$i] : '';
													$phone_val = isset($phones[$i]) ? $phones[$i] : '';
												@endphp
								...
								<div class="form-group row align-items-center mb-3">
									<div class="col-8">
										<label class="mb-0">{{ translate('Show Phone Numbers on Website?') }}</label>
									</div>
									<div class="col-4 text-right">
										<label class="aiz-switch aiz-switch-success mb-0">
											<input type="hidden" name="types[]" value="show_contact_phone">
											<input type="checkbox" name="show_contact_phone"
												@if(get_setting('show_contact_phone', 'on') == 'on') checked @endif>
											<span></span>
										</label>
									</div>
								</div>
